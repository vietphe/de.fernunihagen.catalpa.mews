...
//		int numberOfWord = tokens.size()-punct;
		Collection<POS> poses = JCasUtil.select(aJCas, POS.class);
		Map<String,Integer> posFrequency = new HashMap<>();
			for (POS p: poses) {
				posFrequency.put(p.getCoarseValue(), posFrequency.getOrDefault(p.getCoarseValue(), 0) + 1);
			}
...
//		exportMultiHashMapListToCSV("D:\\HiWi\\LiFT\\output\\CoarsePOS.csv", featureList);
	...
	public static void exportCSVFile(String filePath, ArrayList<Map<String,String>> list) {
		try {
